...
Re = (w_NOCT*Dh)/visc_k; %Reynolds number

Gr = ((g*beta*(t_inoct - Ta)*(Dh^3))/visc_k^2)*sin(theta_m);

h_forced_lam = ((0.86*(Re^-0.5))/(pr_air^0.67))*roh_air*cp_air*w_NOCT;
h_forced_turb = ((0.028*(Re^-0.2))/(pr_air^0.4))*roh_air*cp_air*w_NOCT;

if w_NOCT >= 3
    h_forced = h_forced_turb;
else
    h_forced = h_forced_lam;
end
...
justR = ((alpha*Gm_NOCT) - (h_top*(t_inoct-Ta)) - (epsilon_t*sigma_b*((t_inoct^4)-(t_sky^4)))) / ((h_top*(t_inoct - Ta)) + (epsilon_b*sigma_b*((t_inoct^4)-(Ta^4))));
