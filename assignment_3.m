...
tsky = 0.0552*(Ta^(3/2)); %Temperature of the sky
beta = 1/Ta;
...
h_forced_lam = ((0.86*(Re^-0.5))/(pr_air^0.67))*roh_air*cp_air*w; % Something wrong
...
justR = ((alpha*Gm) - (h_top*(t_inoct-Ta)) - (epsilon_t*sigma_b*((t_inoct^4)-(tsky^4)))) / (h_top*(t_inoct - Ta)) + (epsilon_b*sigma_b*((t_inoct^4)-(tsky^4)));

h_bottom = R * h_top; % Is h_top the same?
